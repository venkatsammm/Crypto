...
function Compare() {
  ...
  const onCoinChange = async (e, isCoin2) => {
    ...
    try {
      ...
      console.log("Selected cryptocurrency:", newCrypto);
  
      if (isCoin2) {
        setCrypto2(newCrypto);
        const data2 = await getCoinData(newCrypto);
        if (data2) {
          console.log("Fetched data for crypto2:", data2);
          settingCoinObject(data2, setCoin2Data);
        } else {
          console.error("No data returned for crypto2");
        }
        const prices1 = await getPrices(crypto1, days, priceType);
        const prices2 = await getPrices(newCrypto, days, priceType);
        if (prices1 && prices2) {
          console.log("Fetched prices for crypto1 and crypto2:", prices1, prices2);
          settingChartData(setChartData, prices1, prices2);
        } else {
          console.error("Failed to fetch prices for crypto1 or crypto2");
        }
      } else {
        setCrypto1(newCrypto);
        const data1 = await getCoinData(newCrypto);
        if (data1) {
          console.log("Fetched data for crypto1:", data1);
          settingCoinObject(data1, setCoin1Data);
        } else {
          console.error("No data returned for crypto1");
        }
        const prices1 = await getPrices(newCrypto, days, priceType);
        const prices2 = await getPrices(crypto2, days, priceType);
        if (prices1 && prices2) {
          console.log("Fetched prices for crypto1 and crypto2:", prices1, prices2);
          settingChartData(setChartData, prices1, prices2);
